...
GEO_accs <- "GSE217700"
download <- "https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE217700&format=file&file=GSE217700%5Fmatrix%5FmRNA%2Ecount%2Etxt%2Egz"
...
glimpse(pData(sm[[1]]))
unique(pData(sm[[1]])$`individual:ch1`)
...
meta <- pData(sm[[1]]) |> 
  ...
  dplyr::rename(id = geo_accession,
                sample_name = `title`,
                patient = `individual:ch1`) |>
  mutate(disease = stringr::str_remove(patient, " .*"),
         age = NA,
         sex = NA,
         dataset = GEO_accs, 
         pediatric = FALSE, 
...
counts <- fread(file_path, data.table = FALSE) |> rename("gene" = "gene_id")
