...
# Failed - only normalised data
GEO_accs <- "GSE196350"
download <- "https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE196350&format=file&file=GSE196350%5Fnorm%5FSIG%5F2020%5F190122%5F2%2Etxt%2Egz"
...
meta <- pData(sm[[1]]) |> 
  ...
  dplyr::rename(id = geo_accession,
                sample_name = `title`,
                sex = `Sex:ch1`) |>
  mutate(disease = ifelse(`disease state:ch1` == "infected", "influenza", "healthy"),
         age = NA,
         dataset = GEO_accs, 
         pediatric = FALSE, 
         source = "whole_blood") |>
