...
GEO_accs <- "GSE230426"
download <- "https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE230426&format=file"
...
unique(pData(sm[[1]])$`disease state:ch1`)
...
meta <- pData(sm[[1]]) |> 
  ...
  dplyr::rename(id = geo_accession,
                sample_name = title,
                timepoint = `Stage:ch1`) |>
  mutate(disease = "staphylococcus_aureus_infection",
         group = paste(`disease state:ch1`, timepoint, sep = "_"),
         age = NA,
         sex = NA,
         dataset = GEO_accs, 
         pediatric = FALSE,   # has on adult with unknown age
         source = "PBMC"
         ) |>
...
colnames(counts) <- stringr::str_remove(colnames(counts), "_.*")

meta_filtered <- meta |>
  dplyr::filter(timepoint == "BEFORE THERAPY")
