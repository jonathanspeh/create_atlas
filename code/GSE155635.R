...
GEO_accs <- "GSE155635"
download <- "https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE155635&format=file&file=GSE155635%5Fnorm%5FSIG2019%5F191219%2Etxt%2Egz"
...
meta <- pData(sm[[1]]) |> 
  ...
  dplyr::rename(id = geo_accession,
                sample_name = `title`,
                sex = `Sex:ch1`) |>
  mutate(disease = ifelse(`status:ch1` == "infected", "influenza", "healthy"),
         age = NA,
         dataset = GEO_accs, 
         pediatric = FALSE, 
         source = "whole_blood") |>
